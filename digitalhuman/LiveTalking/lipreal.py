...
class LipReal(BaseReal):
    @torch.no_grad()
    def __init__(self, opt, model, avatar, bg_id: str = 'default'):
        super().__init__(opt)
    ...
    def process_frames(self,quit_event,loop=None,audio_track=None,video_track=None):
        ...
        while not quit_event.is_set():
            ...
            # bg_img_path = f"./data/bgs/{self.bg_id}.png"
            # bg_img = cv2.imread(bg_img_path)
            # image_height, image_width, _ = image.shape
            # bg_img = cv2.resize(bg_img, (image_width, image_height))     # 调整背景图片大小以匹配绿幕图片的大小

            # hsv_image = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)           # 将绿幕图片转换为HSV颜色空间
            # lower_green = np.array([35, 100, 100])                       # 定义绿幕颜色的HSV范围
            # upper_green = np.array([85, 255, 255])
            # mask = cv2.inRange(hsv_image, lower_green, upper_green)      # 创建掩膜
            # mask_inv = cv2.bitwise_not(mask)                             # 反转掩膜，使绿幕区域为黑色，背景区域为白色
            # fg = cv2.bitwise_and(image, image, mask=mask_inv)            # 将绿幕图片中的绿幕区域替换为黑色
            # bg = cv2.bitwise_and(bg_img, bg_img, mask=mask)              # 将背景图片中的背景区域替换为绿幕图片中的非绿幕区域
            # image = cv2.add(fg, bg)                                      # 合并前景和背景
