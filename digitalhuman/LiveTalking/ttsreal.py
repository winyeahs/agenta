...
class BaseTTS:
    ...
    def put_msg_txt(self,msg:str,opt={},eventpoint=None): 
        if len(msg)>0:
            self.msgqueue.put((msg,opt,eventpoint))
...
class EdgeTTS(BaseTTS):
    def txt_to_audio(self,msg):
        text,opt,textevent = msg
        voice = opt.get('voice', 'zh-CN-YunxiaNeural') or 'zh-CN-YunxiaNeural'
        t = time.time()
        asyncio.new_event_loop().run_until_complete(self.__main(voice,text))
